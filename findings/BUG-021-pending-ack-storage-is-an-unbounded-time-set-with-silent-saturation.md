...
**Current status: ✅ Fully fixed.** Pending ACKs are now stored in a bounded ordered table with send timestamps, and inserting beyond capacity evicts the oldest entry instead of silently refusing to track the newest send. Keeping 64 entries rather than firmware's eight-entry table is an intentional host-side capacity choice enabled by additional memory, not a protocol incompatibility; bounded oldest-entry replacement provides the required lifecycle without requiring time-based expiry. See [MAX_PENDING_ACK_CRCS](https://github.com/openhop-dev/openhop_core/blob/fix/all-the-things-core/src/openhop_core/companion/constants.py#L129-L130) and [the FIFO update](https://github.com/openhop-dev/openhop_core/blob/fix/all-the-things-core/src/openhop_core/companion/base_send.py#L1243-L1267). The fix was introduced in [`9882bf8` — `fix(companion): bound the pending-ACK table and evict the oldest entry`](https://github.com/openhop-dev/openhop_core/commit/9882bf8). Rechecked against current Core head [`9ea7269`](https://github.com/openhop-dev/openhop_core/commit/9ea7269a7e7e903fe433b1f952a4026fe3dcc81b), which is a version-only bump after [`9355d08`](https://github.com/openhop-dev/openhop_core/commit/9355d08e21423886a17979c0d8defb891f5d9d72), and current Repeater head [`9688fe7`](https://github.com/openhop-dev/openhop_repeater/commit/9688fe70e44d85f3cec7de5c77dd804d915a7506), whose only later change after [`6aafa7f`](https://github.com/openhop-dev/openhop_repeater/commit/6aafa7fe991b5b3199b18149f84417f8522d94b2) adds observation-only neighbour-link metrics; no regression was found.
