# BUG-094 — A locally authenticated PATH packet is still forwarded
...
PathHelper updates a local ACL client's outbound path but explicitly leaves the packet retransmittable and returns `False`. PacketRouter therefore passes it to the forwarding engine. To fix it, return authenticated `handled=True` and mark do-not-retransmit only after successful MAC verification and complete parsing. The router should then stop the engine. Failed authentication must still permit forwarding as described in BUG-089.
...
Return authenticated `handled=True` and mark do-not-retransmit only after successful MAC verification and complete parsing. The router should then stop the engine. Failed authentication must still permit forwarding as described in BUG-089.
